...
@section('content')
    <div class="flex justify-between">
        <h1 class="text-2xl font-bold font-lobster border-b-2 border-teal-500 mb-6">Add Customer</h1>
    </div>
    ...
    <div class="flex justify-start items-start">
        ...
        <form method="POST" action="{{ route('customers.store') }}"
            ...
            <div class="mb-6">
                ...
                <textarea name="address" rows="3"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg
                                     focus:ring-2 focus:ring-blue-500 focus:outline-none transition">{{ old('address') }}</textarea>
                @error('address')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex justify-between items-center">
                ...
                <a href="{{ route('customers.index') }}" class="text-gray-500 hover:text-gray-700 transition">
                    ← Back
                </a>
